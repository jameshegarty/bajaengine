...
void Object::CheckCollision(TCollisionPacket* colPackage) { 
  // plane data
  int A, B, C;
  DoubleVector3d p1,p2,p3;
  DoubleVector3d pNormal;
  DoubleVector3d pOrigin;
  DoubleVector3d v1, v2;
  
  // from package
  DoubleVector3d source = colPackage->sourcePoint;
  DoubleVector3d eRadius = colPackage->eRadius;
  DoubleVector3d velocity = colPackage->velocity;	
  
  // keep a copy of this as it's needed a few times
  DoubleVector3d normalizedVelocity = velocity;
  normalizeVector(normalizedVelocity);
  
  // intersection data
  DoubleVector3d sIPoint;    // sphere intersection point
  DoubleVector3d pIPoint;    // plane intersection point 	
  DoubleVector3d polyIPoint; // polygon intersection point
